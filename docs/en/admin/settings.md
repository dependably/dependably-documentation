...
These are the in-product, per-organization settings an owner or admin
configures in the web UI (or the API). All writes require the `tenant:configure`
capability; reads require `read:tenant`. They are independent of the
instance-level [configuration](configuration.md) an operator sets.
...
Organization-wide switches every member inherits. The air-gap and MFA flags
**compose with the instance-wide settings** (`AIR_GAPPED`, `REQUIRE_MFA`): if
the operator has enforced either at the instance level, it is on regardless of
the value here, and the UI shows it locked.
...
| `maxUploadBytes` | bytes, or empty | none | Org-wide upload size ceiling (the instance `MAX_UPLOAD_BYTES` still bounds it). |
...
| `airGapped` | `true` / `false` | `false` | Per-org air-gap: no outbound requests; proxy passthrough forced off. Effective state = instance **or** org. |
| `requireMfa` | `true` / `false` | `false` | Require every user in this org to complete MFA enrollment. Effective state = instance **or** org. |
...
> **Signature verification needs operator trust anchors.** Setting any
> `verify*` gate to `warn` or `block` is rejected unless the matching trust
> anchor is configured at the instance level (`Npm:SignatureKeys`,
> `NuGet:SignatureCertificates`, `PyPI:SigstoreRoots` + `PyPI:TrustedPublishers`,
> `Rpm:GpgKey`, `Maven:SignatureKeys`). Until then the UI disables the control.
> See [Upstreams & signatures](upstreams.md).
