...
**Upload limits** set a size ceiling for package uploads, in bytes: one value for
...
**Retention** sets per-organization retention budgets. An empty field means that
...
| **Max OSV score tolerance** | 0.0 to 10.0 (CVSS) | 10.0 | Block a version whose highest vulnerability score exceeds this. 10.0 blocks nothing on score. |
...
(default), **Warn** (verify and flag), or **Block** (refuse versions that fail
verification or are unsigned):
