...
groups. Okta, Microsoft Entra ID (Azure AD), OneLogin, or any SAML-compliant
IdP works.
...
section of the **Authentication** tab in **Settings** lets you switch it off
...
- At second-factor time the user may choose to remember the device, which
  skips the code next time on that device. Disabling MFA revokes all trusted
  devices.
...
2. Upload your IdP's metadata XML. Dependably reads the entity ID, sign-in
   URL, and signing certificate from it automatically, with nothing to copy by
   hand.
...
4. Turn SSO on. Once the test passes, enable it, and your team signs in
   through your IdP from then on.

Role assignment works with no mapping configured: new users land on the
default role, and you can map IdP groups to Dependably roles whenever you're
ready (see [Role mapping](#role-mapping)). Once SSO is verified you can
optionally switch off password login so everyone goes through your IdP.
Dependably keeps password login available until a successful SSO test confirms
...
a Dependably [role](rbac.md). The **Role mapping** section on the
**Authentication** tab in **Settings** holds the mapping table, the
**role claim attribute**, and the **default role**. Each row of the mapping
table pairs an IdP value with a role. The role claim attribute names the claim
to read values from; blank uses a built-in list of common role/group claim
types (Azure AD, Okta, and generic `Role` / `groups`). The default role is
assigned when nothing matches.
...
- Owner is never assignable via SSO; it is above the IdP ceiling in all
  cases.
- Admin requires opt-in. By default the IdP can assign at most Member, and
