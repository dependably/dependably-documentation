...
CI and shared automation an administrator creates a **service token**, which
belongs to the organization rather than to a person. See
...
4. Optionally set **Expires at**. Leave it empty for a token that does not
   expire. A dated token you rotate is safer.
5. Select **Create**.

Dependably shows the token value **once**, with a **Copy** button. Store it in
your tool's credential store or your CI secret manager, because Dependably
keeps only a hash and cannot show the value again. If you lose it, revoke it
