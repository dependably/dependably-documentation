...
- Sort by selecting a column header. The list opens in risk order: advisories
  in the CISA Known Exploited Vulnerabilities Catalog first, then by
...
- **Fixed in**: the version to upgrade to. Dependably resolves the fix of the
  ...
  the advisory names.
- CWE chips list the Common Weakness Enumeration classes the advisory
...
| Skill | Applies when |
| ----- | ------------ |
| `fix-vulnerable-dependency` | Any advisory with a fixed version. A lockfile-aware upgrade recipe, direct and transitive, per ecosystem |
