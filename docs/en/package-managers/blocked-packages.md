...
travels further than the request did, into CI logs, screenshots, and support tickets.
...
| `manual` | An operator blocked this version by hand. | Ask why; an Admin or Owner can unblock it: open **Packages**, then the version, then **Manual block**. |
...
| `provenance` | Signature or attestation verification did not produce a verified result. | Open **Settings**, then **Signatures**, and check the **Trust anchors** section. |
...
reporting to your operator, because it means a listing surface and its
download path disagree.
